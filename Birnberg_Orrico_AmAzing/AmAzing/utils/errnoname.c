...
//   above, and is not compatible with the C++ standard.
...
#    endif
    ...
    if (errno_ < 0 || (long unsigned int)errno_ >= (sizeof(names) / sizeof(*names)))
        return NULL;
    return names[errno_];
}
...
Under the following compilation conditions:
